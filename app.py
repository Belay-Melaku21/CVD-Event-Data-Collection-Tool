...
def login():
    ...
    u = st.text_input("Investigator Full Name (ባለሙያ)")
    p = st.text_input("Access Key", type="password")
    if st.button("Login"):
        if p == "@Belay6669" and len(u) > 2:
            st.session_state.auth = True
            st.session_state.investigator = u
            st.rerun()
        else:
            st.error("እባክዎ ትክክለኛ ስም እና የይለፍ ቃል ያስገቡ")
...
st.sidebar.success(f"ባለሙያ፡ {st.session_state.investigator}")

# እያንዳንዱን ሴክሽን በTabs መክፈል
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "Section 1-2: Admin & Eligibility", 
    "Section 3-4: Demographics & Lifestyle", 
    "Section 5: Clinical Measures", 
    "Section 6-7: Profile & Meds", 
    "Section 8: Outcome & Submit"
])

with st.form("cvd_research_form", clear_on_submit=True):
    
    with tab1:
        st.subheader("Section 1 & 2: Administrative & Eligibility")
        ...
        with col2:
            ...
            age_elig = st.selectbox("2.1. Age ≥18?", ["1=Yes", "2=No"])
            pre_cvd = st.selectbox("2.2. Pre-existing CVD?", ["2=No", "1=Yes"])
            preg_htn = st.selectbox("2.3. Pregnancy-induced HTN?", ["2=No", "1=Yes"])

    with tab2:
        st.subheader("Section 3 & 4: Demographics & Lifestyle")
        ...
        with col3:
            ...
            occ = st.selectbox("3.5. Occupation", ["1=Gov Employee", "2=Merchant", "3=Farmer", "4=Unemployed", "5=Other"])
    ...
    with tab4:
        st.subheader("Section 6 & 7: Biochemical & Treatment")
        dm = st.selectbox("6.1. Diabetes Mellitus", ["1=Yes", "2=No"])
        ckd = st.selectbox("6.2. CKD", ["1=Yes", "2=No"])
        prot = st.selectbox("6.3. Proteinuria", ["1=Positive", "2=Negative"])
        chol = st.text_input("6.4. Cholesterol (mg/dL)")
        comp = st.selectbox("6.5. Baseline Complications", ["1=None", "2=Prior Stroke", "3=Prior Cardiac"])
    ...
    with tab5:
        st.subheader("Section 8: Outcome & Survival")
        ...
        ev_date = st.text_input("8.3. Event Date (E.C.)") if "Yes" in ev else "NA"
        
        censor = st.selectbox("8.4. Censoring Details", ["1=Not Censored", "2=Lost to Follow-up", "3=Died (Non-CVD)", "4=Study ended"])
        censor_date = st.text_input("8.5. Last Follow-up Date (E.C.)") if censor != "1=Not Censored" else "NA"

        st.markdown("---")
        submit = st.form_submit_button("🚀 SUBMIT ALL SECTIONS")
        ...
        if submit:
            df = pd.DataFrame([{
                ...
                "Investigator_Name": st.session_state.investigator, 
                "Timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            }])
            existing = conn.read()
            updated = pd.concat([existing, df], ignore_index=True)
            conn.update(data=updated)
            st.success(f"መረጃው በባለሙያ {st.session_state.investigator} አማካኝነት በትክክል ተመዝግቧል!")
            st.balloons()
